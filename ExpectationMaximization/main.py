...
def expectation_maximization(data: np.ndarray, n_clusters: int, precision: float = 10**-2) -> tuple:
    clusters = np.array( list((np.random.rand(), np.random.rand()**2) for _ in range(n_clusters)) )
    ...
    while change>precision:
        ...
        clusters_prior = clusters
        clusters = np.concatenate([means, var], axis=0).T
        change = np.sqrt(np.sum(np.sum(np.square(clusters - clusters_prior))))
